...
const PrintedFormJoin = ({ printRef, savePDF, targetRef, toPDF }) => {
  const unionTrade =
  ...
  return (
    <div className={s.printedForm} ref={targetRef}>
      <div ref={printRef}>
        <div className={s.joinForm}>
          ...
          <div className={s.body}>
            Прошу принять меня в члены профсоюза работников здравоохранения РФ c{" "}
            {date} . <br />С уставом Профсоюза работников здравоохранения РФ
            ознакомлен
            {isFemale && "a"}.
          </div>
      ...
      <div className={s.paymentForm}>
        ...
        <div className={s.title}>Заявление</div>
        <div className={s.body}>
          На основании ст.28 Федерального закона «О профессиональных союзах, их
          правах и гарантиях деятельности» прошу ежемесячно удерживать из моей
          заработной платы членские профсоюзные взносы в размере 1% (одного
          процента) и перечислять их на счет, профсоюзной организации с {date}.
        </div>
        <div className={s.signatureBlock}>
          {date}
          <div className={s.signature}>___________________ {user}</div>
